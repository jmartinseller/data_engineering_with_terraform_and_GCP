...
resource "google_compute_router" "dataproc_router" {
  ...
  network = google_compute_network.vpc_rede_jw.self_link # Altere para a sua VPC, se necessário
  region  = "us-east1"
...
resource "google_compute_router_nat" "dataproc_nat" {
  ...
  nat_ip_allocate_option = "AUTO_ONLY" # O GCP aloca os IPs automaticamente
