####################################################################################################
# Datasets camada raw
####################################################################################################

resource "google_bigquery_dataset" "raw_uncover" {
  project = var.project
  dataset_id = "raw_uncover"
  description = "Dataset contendo dados brutos sem nenhuma transformação"
  labels = local.labels
  location  = var.region
}


####################################################################################################
# Tabelas camada raw
####################################################################################################

resource "google_bigquery_table" "tb_raw_leads_sales" {
  project = var.project
  dataset_id = google_bigquery_dataset.raw_uncover.dataset_id
  table_id  = "tb_raw_leads_sales"
  schema = file("terraform/schemas/tb_leads_sales.json")
  labels = local.labels

 external_data_configuration {
   autodetect    = true
   source_format = "PARQUET" #O formato dos arquivos
   source_uris = [
     "gs://${var.bk_parquet}/leads_sales.parquet",
   ]
 }
}

resource "google_bigquery_table" "tb_raw_tv_radio_influencers" {
  project = var.project
  dataset_id = google_bigquery_dataset.raw_uncover.dataset_id
  table_id  = "tb_raw_tv_radio_influencers"
  schema = file("terraform/schemas/tb_tv_radio_influencers.json")
  labels = local.labels

 external_data_configuration {
   autodetect    = true
   source_format = "CSV" #O formato dos arquivos

   csv_options {
     quote                 = "\""
     field_delimiter       = ","
     skip_leading_rows = 1
   }

   source_uris = [
     "gs://${var.bk_csv}/*.csv",
   ]
 }
}

resource "google_bigquery_table" "tb_raw_public_test_crm_email" {
  project = var.project
  dataset_id = google_bigquery_dataset.raw_uncover.dataset_id
  table_id  = "tb_raw_public_test_crm_email"
  schema = file("terraform/schemas/tb_public_test_crm.json")
  labels = local.labels

  external_data_configuration {
    autodetect    = true
    source_format = "GOOGLE_SHEETS"

    google_sheets_options {
      skip_leading_rows = 1
      range = "email!A:C"
    }

    source_uris = [
      "https://docs.google.com/spreadsheets/d/1a7oLWioF0vzcpuSCbpNjtSvPxyf5HHV5UZCn2f7nQvk/edit?gid=0#gid=0",
    ]
  }
}

resource "google_bigquery_table" "tb_raw_public_test_crm_sms" {
  project = var.project
  dataset_id = google_bigquery_dataset.raw_uncover.dataset_id
  table_id  = "tb_raw_public_test_crm_sms"
  schema = file("terraform/schemas/tb_public_test_crm.json")
  labels = local.labels

  external_data_configuration {
    autodetect    = true
    source_format = "GOOGLE_SHEETS"

    google_sheets_options {
      skip_leading_rows = 1
      range = "sms!A:C"
    }

    source_uris = [
      "https://docs.google.com/spreadsheets/d/1a7oLWioF0vzcpuSCbpNjtSvPxyf5HHV5UZCn2f7nQvk/edit?gid=1135476145#gid=1135476145",
    ]
  }
}

####################################################################################################
# Datasets camada prep
####################################################################################################

resource "google_bigquery_dataset" "prep_uncover" {
  project = var.project
  dataset_id = "prep_uncover"
  description = "Dataset contendo dados Prontos para serem usados"
  labels = local.labels
  location  = var.region
}


####################################################################################################
# Tabelas camada prep
###################################################################################################

resource "google_bigquery_table" "tb_prep_leads_sales" {
  project = var.project
  dataset_id = google_bigquery_dataset.prep_uncover.dataset_id
  table_id  = "tb_prep_leads_sales"
  schema = file("terraform/schemas/tb_leads_sales.json")
  labels = local.labels
}

resource "google_bigquery_table" "tb_prep_tv_radio_influencers" {
  project = var.project
  dataset_id = google_bigquery_dataset.prep_uncover.dataset_id
  table_id  = "tb_prep_tv_radio_influencers"
  schema = file("terraform/schemas/tb_tv_radio_influencers.json")
  labels = local.labels
}

resource "google_bigquery_table" "tb_prep_public_test_crm_email" {
  project = var.project
  dataset_id = google_bigquery_dataset.prep_uncover.dataset_id
  table_id  = "tb_prep_public_test_crm_email"
  schema = file("terraform/schemas/tb_public_test_crm.json")
  labels = local.labels
}

resource "google_bigquery_table" "tb_prep_public_test_crm_sms" {
  project = var.project
  dataset_id = google_bigquery_dataset.prep_uncover.dataset_id
  table_id  = "tb_prep_public_test_crm_sms"
  schema = file("terraform/schemas/tb_public_test_crm.json")
  labels = local.labels
}
